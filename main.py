...
sio = socketio.AsyncClient(ssl_verify=False, engineio_logger=False, logger=False)
mons = []
...
async def monitorList(data):
    for i in data:
        if data[i]["active"] == 1:
            ...
            #print("monitorList", {"id": data[i]["id"], "name": data[i]["name"], "url": data[i]["url"]})


@sio.event
async def heartbeatList(id, data, lol):
    for i in mons:
        if i["id"] == data[0]["monitor_id"]:
            try:
                i["uptime"] = data
            except KeyError:
                pass
...
async def uptime(monitor_id, data, uptime_time):
    #print(monitor_id, data, uptime_time, "HALLO")
    if data == 720:
        for i in mons:
